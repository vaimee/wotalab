...
In this exercise, you will upgrade the Semaphore Thing so that its state is no longer simulated in memory, but instead stored in and retrieved from a Modbus-based emulated physical device.  
...
## Step 2: Implement Modbus Semaphore Class
...
>> Once implemented, instantiate this class in src/main.ts alongside the Counter Thing, before starting the orchestrator.
